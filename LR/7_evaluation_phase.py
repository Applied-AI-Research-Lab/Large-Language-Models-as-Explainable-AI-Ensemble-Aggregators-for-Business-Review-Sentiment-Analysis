# python3 LR/7_evaluation_phase.py --predictions-dir LR/predictions
...
EXPECTED_FILES = [
    "preds_lr_balanced_on_balanced.csv",
    "preds_lr_natural_on_natural.csv",
    "preds_lr_balanced_on_natural.csv",
    "preds_lr_natural_on_balanced.csv",
]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluation phase for LR model family.")
    parser.add_argument("--predictions-dir", type=Path, default=Path("LR/predictions"))
    parser.add_argument("--tuning-base-dir", type=Path, default=Path("LR/tuning_outputs"))
    parser.add_argument("--output-dir", type=Path, default=Path("LR/evaluation_outputs"))
    ...
    if pred_rows:
        ...
        pred_df.to_latex(
            ...
            index=False,
            caption="LR prediction performance across evaluation files.",
            label="tab:lr_prediction_metrics",
            float_format="%.4f",
        )
    ...
    for pipeline in ["balanced", "natural"]:
        ...
        if not top.empty:
            ...
            top.to_latex(
                ...
                index=False,
                caption=f"LR top-{args.top_k_trials} tuning trials ({pipeline}).",
                label=f"tab:lr_top_trials_{pipeline}",
                float_format="%.4f",
            )
    ...
    if summary_rows:
        ...
        summary_df.to_latex(
            ...
            index=False,
            caption="LR best tuning configuration summary per pipeline.",
            label="tab:lr_tuning_summary",
            float_format="%.4f",
        )
