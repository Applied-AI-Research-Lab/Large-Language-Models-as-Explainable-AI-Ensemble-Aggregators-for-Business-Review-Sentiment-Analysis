# python3 NB/7_evaluation_phase.py --predictions-dir NB/predictions --tuning-base-dir NB/tuning_outputs --output-dir NB/evaluation_outputs
import argparse
import json
...
EXPECTED_FILES = [
    "preds_nb_balanced_on_balanced.csv",
    "preds_nb_natural_on_natural.csv",
    "preds_nb_balanced_on_natural.csv",
    "preds_nb_natural_on_balanced.csv",
]
        ...
        rows.append(
            {
                ...
                "fit_prior": params.get("fit_prior"),
            }
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluation phase for NB model family.")
    parser.add_argument("--predictions-dir", type=Path, default=Path("NB/predictions"))
    parser.add_argument("--tuning-base-dir", type=Path, default=Path("NB/tuning_outputs"))
    parser.add_argument("--output-dir", type=Path, default=Path("NB/evaluation_outputs"))
    ...
    if pred_rows:
        ...
        pred_df.to_latex(
            ...
            index=False,
            caption="NB prediction performance across evaluation files.",
            label="tab:nb_prediction_metrics",
            float_format="%.4f",
        )
    ...
    for pipeline in ["balanced", "natural"]:
        ...
        if not top.empty:
            ...
            top.to_latex(
                ...
                index=False,
                caption=f"NB top-{args.top_k_trials} tuning trials ({pipeline}).",
                label=f"tab:nb_top_trials_{pipeline}",
                float_format="%.4f",
            )
    ...
    if summary_rows:
        ...
        summary_df.to_latex(
            ...
            index=False,
            caption="NB best tuning configuration summary per pipeline.",
            label="tab:nb_tuning_summary",
            float_format="%.4f",
        )
