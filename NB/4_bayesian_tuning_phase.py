# python3 NB/4_bayesian_tuning_phase.py --pipeline balanced --n-trials 250 --startup-trials 25 --alpha-max 20.0 --fit-prior-mode search --output-dir NB/tuning_outputs/balanced_wider_250
# python3 NB/4_bayesian_tuning_phase.py --pipeline natural --n-trials 250 --startup-trials 25 --alpha-max 20.0 --fit-prior-mode search --output-dir NB/tuning_outputs/natural_wider_250
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Bayesian tuning for TF-IDF + Naive Bayes.")
    ...
    parser.add_argument(
        "--fit-prior-mode",
        choices=["true_only", "false_only", "search"],
        default="search",
    )
    ...
    output_dir = args.output_dir or Path(f"NB/tuning_outputs/{args.pipeline}")
    ...
    def objective(trial: optuna.trial.Trial) -> float:
        ...
        if args.fit_prior_mode == "true_only":
            fit_prior = True
        elif args.fit_prior_mode == "false_only":
            fit_prior = False
        else:
    ...
    payload = {
        "run_metadata": {
            ...
            "fit_prior_mode": args.fit_prior_mode,
        },
        "summary": {
